...
data_2019 <- left_join(data_2019, continents, by = "country")
```
...
data_2019 <- left_join(data_2019, data_gdp1, by = "country")
...
americas_fit <- linear_reg() %>%
  set_engine("lm") %>%
  fit(life_ladder ~ life_expectancy + social_support + gdp_2019, data = (filter(data_2019, continent == "Americas")))
tidy(americas_fit)
...
europe_fit <- linear_reg() %>%
  set_engine("lm") %>%
  fit(life_ladder ~ life_expectancy + social_support + gdp_2019, data = (filter(data_2019, continent == "Europe")))
tidy(europe_fit)
...
africa_fit <- linear_reg() %>%
  set_engine("lm") %>%
  fit(life_ladder ~ life_expectancy + social_support + gdp_2019, data = (filter(data_2019, continent == "Africa")))
...
asia_fit <- linear_reg() %>%
  set_engine("lm") %>%
  fit(life_ladder ~ life_expectancy + social_support + gdp_2019, data = (filter(data_2019, continent == "Asia")))
